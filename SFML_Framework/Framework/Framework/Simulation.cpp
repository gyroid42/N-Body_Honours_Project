...
bool Simulation::GenerateAsteroids(int numAsteroids) {
	...
	if (ADD_RANDOM_BODIES) {
		...
		for (int i = 0; i < numAsteroids; i++) {
			...
			newAsteroid->Init(sf::Vector2f(static_cast <float> (rand()) / (static_cast <float> (RAND_MAX / RANDOM_BODY_MAX_X)), static_cast <float> (rand()) / (static_cast <float> (RAND_MAX / RANDOM_BODY_MAX_Y))), sf::Vector2f(0.0f, 0.0f), RANDOM_BODY_MASS);
	...
	if (ADD_ORBIT_BODIES) {
		...
		planet->Init(sf::Vector2f(500.0f, 500.0f), sf::Vector2f(0.0f, 0.0f), 1.0E17f);
		planet->SetColour(sf::Color::Red);
		bodies_.push_back(planet);

		Body* satelite = new Body();
		satelite->Init(sf::Vector2f(600.0f, 500.0f), sf::Vector2f(0.0f, 25.83215f), 10);
		satelite->SetColour(sf::Color::Cyan);
		bodies_.push_back(satelite);

		Body* satelite2 = new Body();
		satelite2->Init(sf::Vector2f(400.0f, 500.0f), sf::Vector2f(0.0f, -25.83215f), 10);
		satelite2->SetColour(sf::Color::Cyan);
		bodies_.push_back(satelite2);

		Body* satelite3 = new Body();
		satelite3->Init(sf::Vector2f(500.0f, 600.0f), sf::Vector2f(-25.83215f, 0.0f), 10);
		satelite3->SetColour(sf::Color::Cyan);
		bodies_.push_back(satelite3);

		Body* satelite4 = new Body();
		satelite4->Init(sf::Vector2f(500.0f, 400.0f), sf::Vector2f(25.83215f, 0.0f), 10);
